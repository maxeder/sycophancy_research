                ...
                for is_pro in [True, False]:
                    ...
                    for rep in range(1 if TESTING else config.USERDATAGEN_N_REPLICATIONS):
                        ...
                        try:
                            ...
                            # Fetch turns from LLM
                            prompts_list = generate_response(topic_data['question'], stance_text, is_pro, attribution)

                            if not isinstance(prompts_list, list) or len(prompts_list) != config.USERDATAGEN_N_TURNS:
                                raise ValueError(f"Expected a list of {config.USERDATAGEN_N_TURNS} prompts, got {type(prompts_list)}: {prompts_list}")
