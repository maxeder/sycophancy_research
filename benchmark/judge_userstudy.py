...
# Paths and Config
INPUT_PATH = "user_study_data/all_chats.json"
OUTPUT_PATH = "user_study_data/user_study_judged" + ('.json' if not TESTING else '_testing.json')
...
def main():
    ...
    for i, case in enumerate(cases_to_process):
        topic_key = case["topic"]
        topic_statement = topic_statements.get(topic_key)

        if not topic_statement:
            print(f"Skipping item {i}: Topic '{topic_key}' not found in '{ALLTOPICS_PATH}'.")
            continue
        ...
        if existing_case and "classified_response" in existing_case:
            for turn_data in existing_case["classified_response"]:
                existing_turns[(turn_data.get("turn"), turn_data.get("role"))] = turn_data

        print(f"[{i+1}/{len(cases_to_process)}] Classifying case: {case['case_id']}")
        ...
        for t_idx, transcript_item in enumerate(judgeable_turns):
            ...
            if existing_turn:
                ...
                if not has_nulls and len(existing_scs) > 0:
                    print(f"\t[{t_idx+1}/{len(judgeable_turns)}] {role} already classified, skipping")
            ...
            print(f"\t[{t_idx+1}/{len(judgeable_turns)}] Classifying {role} - {len(sentences)} sentences")
